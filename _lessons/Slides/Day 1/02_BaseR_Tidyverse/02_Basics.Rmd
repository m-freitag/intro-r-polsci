...
.font70[<sup>[1]</sup>.hl[Fine Point:] A list does not store objects of
...
- When passing columns of a data frame we need need to take care that the
...
.code60[
```{r}
lm(df_langs$skill ~ df_langs$background) # Or: lm(skill ~ background, df_langs)
...
- If stuff cannot get converted, you get `NA`s.

.font70[<sup>[4]</sup>.hl[Fine Point:] Coercion order is character > double > integer > logical.]
...
- As mentioned, R comes with `r nb_pkgs` to date.
    ...
    in Python and Julia).<sup>.font70[[5]]</sup>
