...
- Useful for...
    - exploring data structure/cleaning data (e.g. spotting missings or outliers)
    - identfying trends, clusters, descriptive patterns
...
data (e.g. graphical representations of database structures or DAGs as a tool to see conditions for causal identification more easily)
...
- Based on what I see in the data, does this accurately represent what I would see in the population? How certain am I?

- .hl[Ex]: What is $E[Y|X]$ in population given I see $E[y|x]$ in sample? 

- Visualizing uncertainty about parameters...
...
- John Snow did not and forensically partialled out contaminated water as the cause of infection.
...
3D when there is no third dimensions. Also avoid 3D charts at all cost/break down to multiple 2D charts.
...
- We will not go through types of charts systematically but will get to know several when learning `ggplot2`
...
.code60[
```{r}
unvotes <- import("data/UNVotes.parquet")
sessions <- unvotes %>%
  select(session, date) %>%
  group_by(session) %>%
...
Working and visualizing spatial data (i.e. [GIS stuff](https://en.wikipedia.org/wiki/Geographic_information_system)) is a pretty complex subject.

You got to think of [projections](https://en.wikipedia.org/wiki/Map_projection),
the [use of color](https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3), [spurious/meaningless spatial correlations](https://xkcd.com/1138/), and alot more... 
